ActiveAdmin.register_page 'push_service', namespace: :admin do
  menu label: 'Сервис Уведомлений', priority: 2, parent: 'Уведомления'
  content title: 'Сервис Уведомлений' do
    div(class: 'grid grid-cols-2 gap-2') do
      div do
        panel 'Уведомления  на Компанию' do
          active_admin_form_for :push_notification, url: :admin_push_service_send_notification, builder: ActiveAdmin::FormBuilder do |f|
            f.inputs do
  ...
  page_action :send_notification, method: :post do
    ...
    redirect_to admin_push_service_path, notice: 'Ваш сообщение отправлено'
  end
end
