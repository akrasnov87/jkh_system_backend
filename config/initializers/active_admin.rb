ActiveAdmin.setup do |config|
  ...
  config.namespace :staff do |ns|
    ns.root_to = 'tickets#index'
    ns.authorization_adapter = 'StaffAuthorizationAdapter'
  end

  config.namespace :admin do |ns|
    ns.root_to = 'accounts#index'
    ns.authorization_adapter = 'AdminAuthorizationAdapter'
  end

  config.namespace :super_admin do |ns|
    ns.root_to = 'users#index'
    ns.authorization_adapter = 'SuperAdminAuthorizationAdapter'
  end

  class StaffAuthorizationAdapter < ActiveAdmin::AuthorizationAdapter
    def authorized?(_action, _subject = nil, _controller = nil)
      user.employee?
    end
  end

  class SuperAdminAuthorizationAdapter < ActiveAdmin::AuthorizationAdapter
    def authorized?(_action, _subject = nil, _controller = nil)
      user.super_admin?
    end
  end
  ...
  config.csv_options = { col_sep: ';', encoding: Encoding::UTF_8}
  #
